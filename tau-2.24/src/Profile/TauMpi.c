...
#ifdef DEBUG
  fprintf( stderr, "my_MPI_Send_init call on %d\n", procid_0 ); 
#endif /* DEBUG */
...
MPI_Request * request;
{
  ...
  int typesize; 
  int dest;
  MPI_Comm_rank(comm, &dest);
  PMPI_Type_size( datatype, &typesize );
  sprintf(log_string,"MPI_Irecv() from %d to %d, size %d", source , dest, typesize*count);

  TAU_PROFILE_TIMER(tautimer, log_string,  " ", TAU_MESSAGE);
  TAU_PROFILE_START(tautimer);
...
MPI_Status * status;
{
  ...
  MPI_Comm_rank(comm, &dest);
  PMPI_Type_size( datatype, &typesize );
  sprintf(log_string,"MPI_Recv() from %d to %d, size %d", source , dest, typesize*count);
...
MPI_Comm comm;
{
  ...
  PMPI_Type_size( datatype, &typesize );
  sprintf(log_string,"MPI_Send() from %d to %d, size %d", source , dest, typesize*count);
